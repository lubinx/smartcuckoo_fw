...
static void MSG_common_key(struct PANEL_runtime_t *runtime, enum PANEL_message_t msgid)
{
    ...
    case MSG_OK_BUTTON:
        if (runtime->tmp_content.en)
        {
            ...
            mqueue_postv(runtime->mqd, MSG_OK_BUTTON, 0, 0);
        }
        break;
...
static void GPIO_button_callback(uint32_t pins, struct PANEL_runtime_t *runtime)
{
    if (PIN_EXTIO == (PIN_EXTIO & pins))
    {
        ...
        {
            mqueue_flush(runtime->mqd);
            mqueue_postv(runtime->mqd, MSG_SNOOZE_BUTTON, 0, 0);
        }
        if (PIN_MESSAGE == (PIN_MESSAGE & pins))
        {
            mqueue_flush(runtime->mqd);
            mqueue_postv(runtime->mqd, MSG_MESSAGE_BUTTON, 0, 0);
        }
    #endif
}
...
static void IrDA_callback(struct IrDA_nec_t *irda, bool repeat, struct PANEL_runtime_t *runtime)
{
    ...
    if (0xFF == (irda->addr + irda->addr_inv) && 0xFF == (uint8_t)(irda->cmd + irda->cmd_inv))
    {
        ...
        {
        case 0xFA:
            msgid = MSG_SNOOZE_BUTTON;
            break;

        // function key
        case 0xDC:
            msgid = MSG_SETTING_BUTTON;
            break;
        case 0x82:
            msgid = MSG_NOISE_BUTTON;
            break;
        case 0x80:
        ...
        case 0x81:
            ...
            break;
        case 0x8D:
            msgid = MSG_RECORD_BUTTON;
            break;
        case 0xD0:
            msgid = MSG_LAMP_EN_BUTTON;
            break;
        case 0x95:
            msgid = MSG_LAMP_BUTTON;
            break;
        ...
        case 0xCA:
            ...
            break;
        case 0xD2:
            msgid = MSG_DOWN_BUTTON;
            break;
        case 0x99:
            msgid = MSG_LEFT_BUTTON;
            break;
        case 0xC1:
            msgid = MSG_RIGHT_BUTTON;
            break;
        case 0xCE:
            msgid = MSG_OK_BUTTON;
            break;
